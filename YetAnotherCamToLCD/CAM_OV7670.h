...
class pw_OV7670    // : public I2SCamera
{
  ...
  public:
    ...
    intr_handle_t vSyncInterruptHandle;
    int mode;
    ...
    // Offset value 0-3, used only in a failed attempt to understand what is does.
    void frameControl(int hStart,  int vStart, int offset)
    {
      int hStop = (hStart + 640) % 784;
      writeRegister(REG_HSTART, hStart >> 3);
      writeRegister(REG_HSTOP,  hStop >> 3);
      int hr76 = (offset & 0b011) << 6;
      writeRegister(REG_HREF, hr76 | ((hStop & 0b111) << 3) | (hStart & 0b111));
    ...
    void setCeilingAGC(int agcCeiling) // Range 0-6
    { agcCeiling = agcCeiling % 7;
      int otherBits = readRegister(REG_COM9) & 0x8F;
      int newVal = otherBits | (agcCeiling << 4);
      writeRegister(REG_COM9, newVal);
    }

    void toggleBits(int reg,  int bits) {
      unsigned int mask = ~bits;
      int val = readRegister(reg);
      int unchanged = val & mask;
      int changes = (val ^ bits) & bits;
      int newVal = unchanged | changes;
      Serial.printf("Changed camera reg %02x to %02x\n", reg, newVal);
    ...
    bool initVSyncInterrupt(int pin,  void (*vSyncInterrupt)(void *))
    {
      gpio_num_t vSyncPin = (gpio_num_t)pin;
    ...
    void setMode(int mode, bool autoStart) {       // 0,1 or 2 for QQQ, QQ, or Q VGA.  All are RGB565
      ...
      writeRegister(REG_COM10, 0x20);   // PCLK does not toggle during horizontal blank
    ...
    // Clock timing, do not change anything except xclk_hz
    const long  xclk_hz = 20000000;  // 20Mhz is 25 fps.
    const long t_pclk = 1000 / (xclk_hz/1000000);  // in nanosecs. e.g. 50ns.   Names follow the datasheet  
    const long t_p = 2 * t_pclk;                   // RGB is 2 bytes per pixel, so this is the pixel time in ns  
    const long t_line = 784 * t_p; 
    const long t_vsync_micros = (510 * t_line) / 1000;      // Time from one vsync until the next one

    // I'm going to use the countDownDuration after a VSYNC for other computation.  So this timing gives me
    // a deadline to tell if I got everyting done before the next pixel data was due to arrive.                
    const long countDownDuration_micros = (20 * t_line) / 1000;  
    // Time from a vsync until first HREF goes high (in VGA mode). 
    // In subsampled QVGA mode we actually have an extra one or two linetimes 
    // from what I can see on a scope. 
      
  
    pw_OV7670(const int SIOD, const int SIOC, const int XCLK, const int VSYNC, void (*vSyncListener)(void* arg))
    {
      ...
      Serial.printf("xclk=%ldHz;  t_pclk=%ldns;  t_p=%ldns;  t_line=%ldns;\ncountDownDuration=%ldus;  t_vsync=%ldus;  expected fps=%.1f;\n",
          xclk_hz, t_pclk, t_p, t_line, countDownDuration_micros, t_vsync_micros, 1000000.0/t_vsync_micros);   

      while (true) {
        bool succeeded = true;
        ClockEnable(XCLK, xclk_hz);  // base is 80MHz, at 20MHz I get 25 VSYNCs per sec
        ...
        // Ver 1.4 spec sheet says limits for clock are 10MHz - 48MHz.
      ...
      // Once we can talk to the camera using SCCB, bind the VSYNC interrupt handler.
      // Interrupts are not enabled until we start() the camera.
      initVSyncInterrupt(VSYNC, vSyncListener);
    }
